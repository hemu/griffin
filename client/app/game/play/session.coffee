...
ui = require './game-ui'
# might need to change name since it is confusing with
# world variable
world = require './world'
config = require './game-config'
cam = require './game-camera'
...
class SessionHost
  ...
  constructor: (@game) ->
    ...
    # Add all sprites in play to this group so that the ui.GameUI's group sits
  ...
  initialize: (player_configs) ->
    ...
    input.GameInput.shost = this
    input.GameInput.setupInputs()
    ui.GameUI.initialize(this)
    ...
    bgX = @game.world.width / @background.width / config.GameConstant.cameraScale
    bgY = @game.world.height / @background.height / config.GameConstant.cameraScale
    ...
    @gcamera = new cam.GameCamera(this)
    @gcamera.initialize(1.0)
    @endPlayerTurn()

    ui.GameUI.bringToTop()
  ...
  update: (dt) ->
    ...
    if @turn_time_remaining > 0
      ...
      if oldtime != newtime
        ui.GameUI.updateTurnTime(newtime)
      if @turn_time_remaining <= 0
        ui.GameUI.updateTurnTime(0)
  ...
  tryEndPlayerTurn: (died=false) ->
    ...
    @endingTurn = true
    @endTurnTimer = config.GameConstant.endTurnWaitTime
